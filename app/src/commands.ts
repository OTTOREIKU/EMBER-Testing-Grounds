...
type TableKind =
  ...
  | 'callDefense' | 'answerDefense' | 'clearDefense' | 'setCombatView' | 'focusAnswer' | 'focusReroll' | 'kcArmor' | 'designateHit' | 'meleeEvade'
...
export function apply(data: GameData, state: GameState, cmd: Command): void {
  ...
  if (cmd.kind === 'focusAnswer' || cmd.kind === 'focusReroll' || cmd.kind === 'kcArmor' || cmd.kind === 'designateHit' || cmd.kind === 'meleeEvade') {
