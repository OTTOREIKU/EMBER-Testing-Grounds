...
// The tether sweep measures no distance without a board, but a partner gone
// from the table still ends the Tether there: the pad's never ended at all
// (audit Phase 4, H1). It used to return at the flag.
check('the tether sweep measures no distance without a board',
  /const apart = !state\.noBoard && !!other && rangeBetween\(t, other\)\.range > link\.range;/.test(units), true);
check('but still cuts a partner gone from the table',
  /if \(other && standing\(other\) && standing\(t\) && !apart\) continue;/.test(units), true);
...
// A free or granted one marks the Opportunity moved instead, since the table
// said the unit moved (audit Phase 4, E1).
check('a boardless Maneuver spends the Tick outright',
  /case 'maneuver': \{[\s\S]{0,1400}?if \(state\.noBoard\) \{[\s\S]*?sc\.opp = lockStance\(t, spendManeuver\(o0\)\);[\s\S]{0,400}?else if \(o0 && sc\) sc\.opp = \{ \.\.\.o0, moved: true \};\s*\n\s*takeMoveGrant\(state, cmd\);\s*\n\s*return;/.test(cmds), true);
// One Non-Humanoid charge, paid before the branch (nonhumanoid.test counts it).
// And Obstruct's Link beside it, above the same branch (audit Phase 4, D2).
check('and the Link is paid once, above the branch',
  /const cost = nonHumanoidCost\([^\n]*\n\s*if \(cost > 0\)[^\n]*\n(?:\s*\/\/[^\n]*\n)*\s*if \(cmd\.breakAwayLink\)[^\n]*\n\s*if \(state\.noBoard\) \{/.test(cmds), true);
