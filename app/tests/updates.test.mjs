...
check('pad: under EMBER PAD, screens before a table only',
  pad.includes("when: () => screen !== 'table'") && pad.includes(".pad-head')?.after(notice)"), true);
...
check('match centre: the top of the lobby column', match.includes(".mc-col')?.prepend(notice)"), true);
